...
#include "methods/Continued_Fractions/Con_Frac.h"
...
int main() {
  ...
  vector<fp_t> computed_moduli =
  find_moduli_roots_by_graeffe(coefficients_for_graeffe, TOLERANCE); cout <<
  "Вычисленные модули корней: "; if (computed_moduli.empty()) { cout << "метод
  не нашел корней";
  }
  else {
  ...
  std::vector<double> coeffs = {-6.0, 11.0, -6.0,
                                1.0}; //{ -6.0, 11.0, -6.0, 1.0 };

  // Вызов метода Лагерра
  // Вызов метода Лагерра
  auto roots = find_roots_by_Continued_Fractions(
      coeffs, numeric_constants::adaptive_epsilon<double>(
