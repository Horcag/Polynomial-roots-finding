...
Генератор многочлена высокой степени:
    ...
    Смещение от границ задавать не нужно, так как центры равномерно
автоматически распределяются Радиусы могут автоматически уменьшиться, чтобы
    ...
    Кратные корни и стандартные вещественные распределяются равномерно
распределяются по свободным интервалам между кластерами Комплексные корни
создаются сопряженными парами
...
//  - real_roots_repeated: все вещественные корни с учётом кратности (для
//  умножения)
...
void generate_high_degree_polynomial(
    unsigned P, unsigned num_complex_pairs, unsigned num_clusters,
    const std::vector<unsigned>
        &cluster_counts_in, // простые корни в каждом кластере (может быть
                            // пустым)
    const std::vector<T>
        &cluster_radii_in, // радиусы кластеров (может быть пустым)
    const std::vector<std::pair<unsigned, unsigned>>
        &multiplicity_groups, // (mult, count)
    T default_cluster_radius, bool normalize_coeffs, std::uint64_t seed,
    std::vector<T> &coefficients, std::vector<T> &real_roots_repeated,
  ...
  for (size_t i = 0; i < cluster_radii_in.size(); ++i) {
    if (cluster_radii_in[i] <= T(0)) {
      throw std::invalid_argument(
          "Радиус кластера должен быть положительным (cluster " +
          std::to_string(i) + ")");
    }
  }
  ...
  T total_radius =
      std::accumulate(cluster_radii_in.begin(), cluster_radii_in.end(), T(0));
  if (total_radius >= T(1)) {
    throw std::invalid_argument(
        "Сумма радиусов всех кластеров не должна превышать 1");
  }

  // инициализация случайного генератора
  std::random_device rd;
  std::uint64_t used_seed =
      (seed != 0) ? seed : static_cast<std::uint64_t>(rd());
  std::mt19937_64 gen(used_seed);

  // подготовка векторов кластеров и радиусов
  // если входные векторы не совпадают num_clusters по размеру, заполняем
  // радиусами по умолчанию
  std::vector<unsigned> cluster_counts;
  if (num_clusters == 0)
    cluster_counts.clear();
  else if (cluster_counts_in.size() == num_clusters)
    cluster_counts = cluster_counts_in;
  else
    cluster_counts.assign(num_clusters, 1u);

  std::vector<T> cluster_radii;
  if (num_clusters == 0)
    cluster_radii.clear();
  else if (cluster_radii_in.size() == num_clusters)
    cluster_radii = cluster_radii_in;
  else
    cluster_radii.assign(num_clusters, default_cluster_radius);

  // копия групп кратностей
  std::vector<std::pair<unsigned, unsigned>> local_mult_groups =
      multiplicity_groups;
  ...
  if (complex_degree > P)
    throw std::invalid_argument(
        "Недостаточная степень P для заданного числа пар комплексных корней");

  unsigned mult_degree = 0;
  for (auto &mc : local_mult_groups) {
    unsigned mult = mc.first;
    unsigned cnt = mc.second;
    if (mult < 1)
      continue;
    // защита от слишком большой кратности
    if (mult > P)
      throw std::invalid_argument(
          "Слишком большая кратность в multiplicity_groups");
    mult_degree += mult * cnt;
  }

  unsigned cluster_simple_roots = 0;
  for (auto c : cluster_counts)
    cluster_simple_roots += c;

  unsigned used_degree = complex_degree + mult_degree + cluster_simple_roots;

  // если не влезает — уменьшаем сначала кластерные простые корни, затем кратные
  // группы
  if (used_degree > P) {
    unsigned overflow = used_degree - P;
    // уменьшаем cluster_counts по очереди, начиная с последних
    for (int i = static_cast<int>(cluster_counts.size()) - 1;
         i >= 0 && overflow > 0; --i) {
    ...
    if (used_degree > P) {
      ...
      // теперь уменьшаем multiplicity группы: удаляем целые корни (каждый
      // удалённый корень уменьшает используемую степень на mult)
      for (auto &mc : local_mult_groups) {
        if (overflow == 0)
          break;
        unsigned mult = mc.first;
        ...
        if (cnt == 0)
          continue;
        // сколько корней этой группы нужно удалить (ceil(overflow / mult)) но
        // не больше, чем cnt
      ...
      mult_degree = 0;
      for (auto &mc : local_mult_groups) {
        mult_degree += mc.first * mc.second;
      }
      used_degree = complex_degree + mult_degree + cluster_simple_roots;
      if (used_degree > P)
        throw std::invalid_argument(
            "Невозможно уместить все запрошенные корни/кратности в степень P "
            "(после попытки сокращения)");
    }
  }

  // оставшиеся простые корни, которые нужно разместить вне кластеров
  unsigned remaining_simple =
      P - (complex_degree + mult_degree + cluster_simple_roots);
  ...
  if (num_clusters > 0) {
    ...
    for (unsigned i = 0; i < num_clusters; ++i) {
      T max_by_border =
          std::min(cluster_centers[i] - T(-1), T(1) - cluster_centers[i]);
      if (cluster_radii[i] > max_by_border)
        cluster_radii[i] = max_by_border;
      if (cluster_radii[i] < T(0))
        cluster_radii[i] = T(0);
    }
    // затем обеспечим отсутствие перекрытия: radius_i <= half distance to
    // neighbors
    for (unsigned i = 0; i < num_clusters; ++i) {
      T left_half = (i == 0) ? (cluster_centers[i] - T(-1))
                             : (cluster_centers[i] - cluster_centers[i - 1]);
      if (i > 0)
        left_half /= T(2);
      T right_half = (i + 1 == num_clusters)
                         ? (T(1) - cluster_centers[i])
                         : (cluster_centers[i + 1] - cluster_centers[i]);
      if (i + 1 < num_clusters)
        right_half /= T(2);
      T allowed = std::min(left_half, right_half);
      if (cluster_radii[i] > allowed)
        cluster_radii[i] = allowed;
    }
  }
  ...
  if (num_clusters == 0) {
    ...
    for (unsigned i = 0; i < num_clusters; ++i) {
      ...
      if (right > left)
        free_intervals.push_back({left, right});
      left = cluster_centers[i] + cluster_radii[i];
    }
    if (left < T(1))
      free_intervals.push_back({left, T(1)});
  }
  // если по каким-то причинам нет свободных интервалов (очень редкий случай),
  // делаем один общий
  if (free_intervals.empty())
    free_intervals.push_back({T(-1), T(1)});

  // формируем список уникальных корней, которые нужно разместить во
  // free_intervals сначала создаём вектор пар (multiplicity, placeholder)
  std::vector<unsigned>
      mults_for_free; // кратности для каждой уникальной корни, которые пойдут в
                      // free_intervals
  for (auto &mc : local_mult_groups) {
    unsigned mult = mc.first;
    unsigned cnt = mc.second;
    for (unsigned k = 0; k < cnt; ++k)
      mults_for_free.push_back(mult);
  }
  // добавляем remaining_simple корней с кратностью 1
  for (unsigned k = 0; k < remaining_simple; ++k)
    mults_for_free.push_back(1u);
  ...
  std::vector<unsigned> counts_per_interval =
      allocate_counts_proportional(M, free_lengths);
  ...
  for (size_t i = 0; i < free_intervals.size(); ++i) {
    ...
    if (k == 0)
      continue;
    auto pts = evenly_spaced_in_interval<T>(free_intervals[i].a,
                                            free_intervals[i].b, k);
  ...
  // если ещё остались неприсвоенные (из-за округлений), добавим их в середину
  // соответствующих интервалов
  while (idx_mults < M) {
    // добавляем в центр первой интервальной области
    unique_real_roots.push_back((free_intervals[0].a + free_intervals[0].b) /
                                T(2));
    real_root_multiplicities.push_back(mults_for_free[idx_mults++]);
  }
  ...
  for (size_t i = 0; i < unique_real_roots.size(); ++i) {
    ...
    for (unsigned k = 0; k < mult; ++k)
      real_roots_repeated.push_back(r);
  }
  ...
  for (auto &r : real_roots_repeated)
    multiply_by_root(coefficients, r);
...
template <typename T>
void print_polynomial(const std::vector<T> &coefficients,
  ...
  if (first_term)
    os << "0"; // если все коэффициенты нулевые
