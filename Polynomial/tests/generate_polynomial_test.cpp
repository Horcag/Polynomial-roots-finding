...
// Вычисление значения полинома (coeffs: младший член первый) в вещественной
// точке x
...
void test_basic_combined() {
  print_test_header("Базовый комбинированный случай (кластеры + кратности + "
                    "комплексные пары)");
  ...
  std::vector<std::pair<unsigned, unsigned>> mult_groups = {
      {2, 1}}; // один корень кратности 2 -> 2 степени
  ...
  cout << "Степень P = " << P
       << ", фактическая степень из коэффициентов = " << (coeffs.size() - 1)
       << "\n";
  ...
  if (normalize && !coeffs.empty()) {
    ...
    cout << "Проверка нормировки: старший коэффициент = "
  ...
  cout << "Уникальных вещественных корней: " << unique_real.size()
       << ", сумма их кратностей = " << sum_mults
       << ", всего повторяющихся вещественных корней (real_repeated) = "
       << real_repeated.size() << "\n";
  ...
  // Проверка точности: для каждого корня P(root) ~ 0 (с допустимой
  // погрешностью)
  ...
  for (size_t i = 0; i < unique_real.size(); ++i) {
    ...
    // Для кратных корней численная ошибка может быть чуть больше; допускаем
...
void test_all_complex() {
  ...
  cout << "Уникальных вещественных корней: " << ur.size()
       << ", повторяющихся вещественных: " << rr.size() << "\n";
  assert(ur.empty());
  assert(rr.empty());
...
void test_clusters_only() {
  ...
  for (double r : ur)
    assert(r >= -1.0 - 1e-12 && r <= 1.0 + 1e-12);
  cout << "кол-во уникальных значений = " << ur.size()
       << ", общее кол-во повторных = " << rr.size() << "\n";
  cout << "ТЕСТ 4: OK\n\n";
}
...
void test_multiplicity_and_overflow_handling() {
  ...
  // плюс ещё некоторые простые/кластеры/комплексы — генератор должен уменьшить
  // запросы или бросить исключение.
  std::vector<std::pair<unsigned, unsigned>> mult_groups = {
      {3, 2}}; // потенциально занимает всю степень
  std::vector<double> coeffs;
  std::vector<double> rr, ur;
  std::vector<unsigned> mults;
  std::vector<std::complex<double>> cz;
  ...
  // Случай 2: просим mult_groups + дополнительные кластерные простые корни ->
  // генератор должен сократить кластерные
  unsigned P2 = 6;
...
void test_invalid_arguments() {
  ...
  try {
    ...
    cout << "Поймано ожидаемое исключение для отрицательного радиуса: "
         << e.what() << "\n";
  }
  ...
  try {
    ...
    assert(
        false &&
        "Ожидалось исключение при недостаточной степени для комплексных пар");
  } catch (const std::invalid_argument &e) {
...
void test_high_degree_numerical_stability() {
  ...
  // Проверка P(r) для вещественных корней (позволим более свободную погрешность
  // для высокой степени)
  double tol = 1e-4;
  for (double r : ur) {
    double v = eval_poly_local(coeffs, r);
    if (std::abs(v) > tol) {
      cout << "Внимание: Погрешность для корня " << r << " = " << v
           << " (> tol=" << tol << ")\n";
    }
    // не assert-им строго, но выводим предупреждение; в идеале должно быть
    // близко к нулю
  }
...
void test_exhaustive_smoke() {
  ...
  // Будем перебирать несколько комбинаций параметров, чтобы покрыть
  // всевозможные пути
      ...
      for (auto nc : clusters) {
        for (auto normalize : norms) {
          for (auto seed : seeds) {
            ...
            std::vector<std::pair<unsigned, unsigned>> mult_groups;
            if (P >= 4)
              mult_groups.push_back({2, 1}); // один двойной корень
            ...
            try {
              ...
              cout << "Допустимая ошибка при комб. параметров (P=" << P
...
int main() {
  ...
  cout << "Запуск полного набора тестов для "
  ...
  try {
    ...
    cerr
        << "Во время тестов произошло непредвиденное неизвестное исключение.\n";
    return 3;
  }
