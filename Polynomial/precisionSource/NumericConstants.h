...
// =============================================================================
// Используются для замены магических чисел (10, 100, 1000 и т.д.)
// при масштабировании epsilon и задания порогов.
...
//
// Для типа double: epsilon() ≈ 2.2e-16
// - COARSE (1e3)  → eps ≈ 2.2e-13 (грубый порог для проверки "почти ноль")
// - STANDARD (1e8) → eps ≈ 2.2e-8  (стандартный порог для методов Сагралова)
// - PRECISE (1e10) → eps ≈ 2.2e-6  (точный порог для методов Грефе)
//
// Для arbitrary precision типов: epsilon() зависит от заданной точности,
// и эти масштабы автоматически адаптируются.

/// Масштаб для грубых проверок (is_zero_val без параметра)
constexpr long long EPSILON_SCALE_COARSE = 1000LL;

/// Масштаб для стандартных методов (Сагралов, ANewDsc) — обеспечивает ~1e-8 для
/// double
...
/// Масштаб для сверхточных проверок (шаг Ньютона) — обеспечивает ~1e-2 для
/// double
 ...
 *
 * ln(DBL_MAX) ≈ 709.8, используем 700 для безопасности.
 ...
 *
 * ln(DBL_MIN) ≈ -708, используем -700 для консистентности.
 */
...
// =============================================================================
// ФУНКЦИИ ДЛЯ ПОЛУЧЕНИЯ КОНСТАНТ В НУЖНОМ ТИПЕ
// =============================================================================
