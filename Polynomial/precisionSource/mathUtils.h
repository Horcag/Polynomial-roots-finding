 ...
 * @param eps Порог для сравнения (по умолчанию: 1000 *
 ...
 *
 * Использует метод наибольших остатков (Hare-Niemeyer), чтобы сумма
 * распределённых частей была в точности равна M.
 *
...
allocate_counts_proportional(unsigned M, const std::vector<double> &weights) {
  ...
  if (sum_w <= 0.0) {
    // Если сумма весов 0, просто кладём всё в первую корзину (или распределяем
    // равномерно?) Выберем первую для определённости.
    if (M > 0)
      result[0] = M;
    return result;
  }
