// Аналог обычному polyprecision, но поддерживающий полиномы от нескольких
// переменных Реализация: Трудолюбов Никита, КМБО-01-22
...
template <class _TY> class mpolynomial {
private:
...
public:
  ...
  // Методы доступа
  const std::map<std::vector<unsigned int>, _TY> &get_terms() const {
    return terms;
  }
  ...
  bool operator==(const mpolynomial &other) const {
  ...
  mpolynomial &operator*=(const mpolynomial &other) {
    ...
    for (const auto &term1 : terms) {
      for (const auto &term2 : other.terms) {
        ...
        if (new_exponents.size() < term2.first.size()) {
          new_exponents.resize(term2.first.size(), 0);
        }
        for (size_t i = 0; i < term2.first.size(); ++i) {
          new_exponents[i] += term2.first[i];
        }
        result.add_term(new_exponents, new_coeff);
      }
    }
  ...
  std::string toString() const {
    ...
    std::vector<std::pair<std::vector<unsigned int>, _TY>> sorted_terms(
    ...
    for (const auto &term : sorted_terms) {
      ...
      if (abs_coeff != _TY(1) || !has_variables) {
        ss << abs_coeff;
      }
      for (size_t i = 0; i < exponents.size(); ++i) {
        if (exponents[i] > 0) {
          // Вот исправленная строка
          if (abs_coeff == _TY(1) && has_variables && i > 0 &&
              !ss.str().empty() && ss.str().back() != ' ')
            ss << "*";
          ss << "x_" << i;
          if (exponents[i] > 1)
            ss << "^" << exponents[i];
...
mpolynomial<_TY> pow(const mpolynomial<_TY> base, int exp) {
  if (exp < 0)
    return mpolynomial<_TY>();
  if (exp == 0)
    return mpolynomial<_TY>(static_cast<_TY>(1));
