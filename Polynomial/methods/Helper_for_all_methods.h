 ...
 *
 * @note Использует a posteriori контроль: проверяет isfinite после деления,
 *       а не пытается предсказать деление на ноль заранее.
 */
template <RealNumber T>
T newton_method(const std::vector<T> &poly, const std::vector<T> &deriv,
                T guess, T epsilon,
  ...
  for (size_t i = 0; i < max_iter; ++i) {
    ...
    // a posteriori контроль: проверяем результат, а не делитель
    // Это надёжнее, чем проверка f_prime < eps заранее
...
void newton_method(std::vector<Complex> &z, const std::vector<Complex> &c,
  ...
  for (Complex &root : z) {
    ...
    // a posteriori контроль результата
...
template <RealNumber T> int sign_changes(const std::vector<T> &coeffs) {
  ...
  for (size_t i = 1; i < coeffs.size(); ++i) {
    // Устранён лишний if: используем арифметико-логическое выражение
    bool is_nonzero = (coeffs[i] != T(0));
    bool sign_changed =
        (prev > T(0) && coeffs[i] < T(0)) || (prev < T(0) && coeffs[i] > T(0));
...
template <RealNumber T> T computeRootBoundCauchy(const std::vector<T> &poly) {
  ...
  for (size_t i = 0; i < actual_degree; ++i) {
    // Устранён if: используем max_val
  ...
  if (is_zero_val(leading_coeff_abs)) {
    throw std::runtime_error(
        "Leading coefficient is zero in computeRootBoundCauchy.");
  }
