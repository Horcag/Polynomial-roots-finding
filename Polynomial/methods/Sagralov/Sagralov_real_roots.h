...
template <RealNumber T> struct SparsePolynomial {
  ...
  explicit SparsePolynomial(const std::vector<T> &dense)
      : SparsePolynomial(dense, numeric_constants::scale_epsilon(
  ...
  SparsePolynomial<T> shift(T a) const {
    ...
    for (const auto &[deg, coef] : terms) {
      ...
      for (int j = 0; j <= deg; ++j) {
        ...
        // Устранён if: используем тернарник для избежания лишнего умножения
        a_pow *= (j < deg) ? a : T(1);
      }
    }
    return result;
...
find_real_roots_by_Sagralov(const std::vector<T> &input,
  ...
  if (mult0 > 0) {
    ...
    results.emplace_back(-static_cast<T>(epsilon), static_cast<T>(epsilon),
                         mult0);
  }
  if (coeffs.size() <= 1)
    return results;
    ...
    if (deg == 1) {
      ...
      if (!is_zero_val(a)) {
        ...
        if (disc > T(0)) {
          ...
          // Сначала находим корень с большим модулем (избегаем потери точности
          // при вычитании близких чисел), затем второй по формуле Виета.
          T root1 =
...
int detail::countSignChanges(const std::vector<std::vector<T>> &seq, T x) {
  ...
  for (auto &p : seq) {
    ...
    if (is_zero_val(v)) { // минимальный сдвиг чтобы не терять знак на корне
      T delta =
          (abs_val(x) > T(0) ? x : T(1)) *
          numeric_constants::scale_epsilon(std::numeric_limits<T>::epsilon(),
                                           numeric_constants::FACTOR_1K);
