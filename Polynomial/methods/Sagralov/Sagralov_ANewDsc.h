...
template <RealNumber T> struct Interval01 {
  ...
  // Конструктор: по умолчанию количество возможных корней = -1 (ещё не
  // вычислено)
 ...
 * где корни в [a,b] отображаются в [0, +∞).
 */
...
std::vector<T> descartes_transform(const std::vector<T> &poly, T a, T b) {
  ...
  for (int i = 0; i <= n; ++i) {
    // (a*y + b)^i
    // Re-initialize numerator for current i
    ...
    // (y + 1)^(n - i)
    // Re-initialize denominator for current n-i
...
int count_sign_changes(const std::vector<T> &coeffs, T eps) {
  ...
  for (const auto &c : coeffs) {
    ...
    // Арифметико-логическое выражение вместо if
...
int test_01(const std::vector<T> &poly, T a, T b, T eps) {
  ...
  // Арифметико-логическое выражение вместо цепочки if
  // v == 0 → 0, v == 1 → 1, иначе → -1
  return (v <= 1) ? v : -1;
}
 ...
 */
template <RealNumber T>
std::optional<T> find_admissible_point(const std::vector<T> &poly, T a, T b,
                                       int grid_size, T eps) {
  T mid = (a + b) / T(2);
  ...
  for (int i = 0; i <= grid_size; ++i) {
    ...
    // Устранён if: используем условное обновление
 ...
 */
template <RealNumber T>
std::optional<Interval01<T>> newton_test(const std::vector<T> &poly,
                                         const Interval01<T> &interval,
                                         int N_param, T eps) {
  ...
  std::vector<T> sample_points = {interval.left + width * T(0.25),
                                  interval.midpoint(),
  ...
  for (const auto &xi : sample_points) {
    ...
    // a posteriori контроль: проверяем результат деления
    ...
    bool in_interval =
        (candidate >= interval.left && candidate <= interval.right);
...
find_real_roots_ANewDsc(const std::vector<T> &input,
  ...
  while (!active_intervals.empty() && iteration < max_iterations) {
    ...
    int test_result =
        anewdsc_detail::test_01(poly, current.left, current.right, epsilon);
    ...
    if (current.variability > 1) {
      auto newton_result =
          anewdsc_detail::newton_test(poly, current, NEWTON_PARAM, epsilon);
    ...
    // Бисекция
    auto m_opt = anewdsc_detail::find_admissible_point(
        poly, current.left, current.right, GRID_SIZE, epsilon);
