...
/**
 * @brief Диск в комплексной плоскости для изоляции корней.
 * @tparam T Вещественный тип, должен удовлетворять концепту RealNumber.
 */
template <RealNumber T> struct Disk {
  cplx<T> center;
  T radius;
  int num_roots;

  // Единый конструктор с параметрами по умолчанию (вместо двух конструкторов)
  Disk(cplx<T> c = cplx<T>(0, 0), T r = T(0), int k = 0)
      : center(c), radius(r), num_roots(k) {}
};

/**
 * @brief Компонент для алгоритма разбиения.
 */
template <RealNumber T> struct Component {
...
// =============================================================================
// ТЕСТЫ ИЗОЛЯЦИИ КОРНЕЙ
// =============================================================================

/**
 * @brief Тест Пелле после N итераций Греффе.
 *
 * @tparam T Вещественный тип.
 * @param a Коэффициенты многочлена.
 * @param m Центр диска.
 * @param r Радиус диска.
 * @return Число корней в диске или -1 при неопределённости.
 */
...
int T_G(const std::vector<cplx<T>> &a, const cplx<T> &m, T r) {
  ...
  // Вычисление N = ceil(log2(1 + log(n) + 5))
  ...
  int N = static_cast<int>(ceil_val(log2_inner));
  N = max_val(1, min_val(N, 12));

  auto c = detail_sagralov::build_Fdelta(a, m, r);

  // Выполняем N итераций Греффе, проверяя условие Пелле после каждой
  for (int iter = 0; iter <= N; ++iter) {
    ...
    const T K = numeric_constants::sagralov_safety_factor_k<T>();
    ...
    if (iter < N) {
      ...
      // Масштабирование степенями двойки для численной устойчивости
      T max_mag = T(0);
      for (const auto &z : c)
...
/**
 * @brief Тест T* с переменным числом итераций Греффе.
 *
 * Адаптивная версия теста Пелле с увеличением числа итераций при неудаче.
 */
template <RealNumber T>
int T_star(const std::vector<cplx<T>> &a, const cplx<T> &m, T r) {
  ...
  // Вычисляем N_base через numeric_limits вместо long double литералов
  T n_val = T(max_val(2, n));
  T log_n = log_val(n_val);
  T inner = T(1) + log_n + T(5);
  T log2_inner = log_val(inner) / log_val(T(2));
  int N_base = static_cast<int>(ceil_val(log2_inner));
  N_base = max_val(1, N_base);

  // Пытаемся с возрастающим N
  for (int N = N_base; N <= min_val(N_base + 6, 18); ++N) {
    ...
    for (int iter = 0; iter <= N; ++iter) {
      ...
      if (iter < N) {
        ...
        // Численно стабильная нормализация
        T max_mag = T(0);
        for (const auto &z : c)
          max_mag = max_val(max_mag, abs_val(z));
...
// =============================================================================
// ТЕСТ НЬЮТОНА И БИСЕКЦИЯ
// =============================================================================

/**
 * @brief Тест Ньютона для сходимости к кластеру корней.
 *
 * @tparam T Вещественный тип.
 */
template <RealNumber T>
std::optional<Disk<T>> NewtonTest(const std::vector<cplx<T>> &a,
                                  const Disk<T> &D, int N_C) {
  if (D.num_roots <= 0)
    return std::nullopt;
  ...
  // a posteriori контроль: проверяем результат деления, а не делитель
  cplx<T> ratio = F_val / Fp_val;
  if (!isfinite_val(std::abs(ratio)))
    return std::nullopt;

  cplx<T> x_new = x_C - cplx<T>(T(k), T(0)) * ratio;
  T r_new = r / T(2 * N_C);
...
/**
 * @brief Бисекция квадрата на 4 подквадрата.
 */
template <RealNumber T>
std::vector<std::pair<cplx<T>, T>> BisectionSquare(const cplx<T> &center,
                                                   T halfw) {
  T h2 = halfw / static_cast<T>(2);
  return {{cplx<T>(center.real() - h2, center.imag() - h2), h2},
...
std::vector<Disk<T>>
CIsolate(const std::vector<T_in> &coeffs_in, cplx<T> center = cplx<T>(0, 0),
         T halfw = static_cast<T>(0.0),
         T min_radius = numeric_constants::adaptive_epsilon<T>(
             numeric_constants::EPSILON_SCALE_COARSE),
         int max_iterations = numeric_constants::DEFAULT_MAX_ITERATIONS * 5) {
  ...
  // a posteriori контроль: если ведущий коэффициент слишком мал, полином
  // вырожден
  if (!isfinite_val(max_coef / leading))
    return {};

  T cauchy_bound = T(1) + max_coef / leading;

  // проблема: CIsolate плохо работает для |z| < 1
  ...
  } else if (cauchy_bound > T(numeric_constants::FACTOR_10)) {
  ...
  if (halfw <= T(0)) {
    ...
    // увеличиваем область для надежности
    halfw = new_cauchy_bound * T(1.8);
  }

  std::vector<Component<T>> work_queue;
  auto process_square = [&](const cplx<T> &sq_center,
                            T sq_half) -> std::optional<Component<T>> {
    // Коэффициент запаса для численной устойчивости
    T safety_factor = numeric_constants::component_radius_safety_factor<T>();
    T radius = sq_half * sqrt_val(static_cast<T>(2)) * safety_factor;
  ...
  for (const auto &[sq_center, sq_half] : initial_subs) {
    ...
    if (comp_opt)
      work_queue.push_back(std::move(*comp_opt));
  }
  ...
  while (!work_queue.empty() && iteration < max_iterations) {
    ...
    if (newton_disk && newton_disk->radius < current.disk.radius * T(0.8)) {
      ...
      work_queue.push_back(std::move(refined));
      continue;
    }

    // NewtonTest не помог, применяем бисекцию
    ...
    for (const auto &[sq_center, sq_half] : subsquares) {
      T safety_factor = numeric_constants::component_radius_safety_factor<T>();
      T radius = sq_half * sqrt_val(T(2)) * safety_factor;
      int k = T_star(a, sq_center, radius);
      if (k > 0)
    ...
    for (size_t i = 0; i < nonempty_squares.size(); ++i) {
      ...
      for (size_t j = i + 1; j < nonempty_squares.size(); ++j) {
        ...
        T dist =
            abs_val(nonempty_squares[i].center - nonempty_squares[j].center);
      ...
      T safety_factor = numeric_constants::component_radius_safety_factor<T>();
      T component_radius =
          max_distance + comp_halfw * sqrt_val(T(2)) * safety_factor;
      ...
      work_queue.push_back(std::move(new_component));
    }
  }
  ...
  if (iteration >= max_iterations && !work_queue.empty()) {
    for (const auto &comp : work_queue) {
      if (comp.disk.num_roots > 0) {
        ...
        isolated_roots.emplace_back(comp.disk.center, safe_radius,
                                    comp.disk.num_roots);
      }
    }
  }

  // обратная трансформация: диски для Q(y) → диски для P(x), делим центры и
  // радиусы на k
  if (scale_k != T(1)) {
