...
// find_moduli_roots_by_graeffe - реализация метода без отсеивания повторяющихся
// корней (порождает кучу мусора) find_moduli_with_multiplicities_by_graeffe -
// отсеивает дублирующиеся корни Реализация: Павлова Анастасия, КМБО-01-22
// vihuhol04@mail.ru
...
/**
 * @brief Вычисление модулей корней методом Греффе (логарифмическая
 * модификация).
 *
...
std::vector<T> find_moduli_roots_by_graeffe(
    ...
    bool debug = false) {
  std::cerr << "[TRACE] Entering find_moduli_roots_by_graeffe. Deg="
            << coeffs_desc.size() - 1 << " MaxIter=" << maxIter << std::endl;

  const T NEG_INF = neg_inf_val<T>();

  // TINY больше не используется — проверка через isfinite_val() вместо v < TINY

  // Безопасный логарифм: использует isfinite для проверки результата
  auto safe_log = [&](const T &x) -> T {
    T v = abs_val(x);
    T result = log_val(v);
    // a posteriori проверка вместо проверки v < TINY
  ...
  // Коэффициент считается "нулевым", если его логарифм не является конечным
  // числом, что автоматически определяется через safe_log и isfinite_val.
  size_t first_nz = 0;
  while (first_nz < coeffs_desc.size() &&
         !isfinite_val(safe_log(coeffs_desc[first_nz])))
    ++first_nz;
  ...
  // Меняем порядок коэффициентов: БЫЛО от старших к младшим → СТАНЕТ от младших
  // к старшим. Это необходимо для корректной работы логарифмической модификации
  // Греффе.
  ...
  if (debug) {
    std::cerr << "[DEBUG] Graeffe: начало, степень полинома = "
              << (p.size() - 1) << ", maxIter = " << maxIter << std::endl;
  }
  ...
  for (int iter = 0; iter < maxIter; ++iter) {
    ...
    auto conv_square_log = [&](const std::vector<T> &Lvec,
      ...
      for (size_t m = 0; m < msize; ++m) {
        ...
        for (size_t i = 0; i < Lvec.size(); ++i) {
          ...
          if (j < Lvec.size()) {
            T lv = Lvec[i] + Lvec[j];
            // Устранена одноразовая переменная sgn: используем emplace напрямую
    ...
    for (size_t m = 0; m < L.size(); ++m) {
      ...
      bool hasPo =
          (m >= 1 && (m - 1) < Lpo2.size() && is_finite_ld(Lpo2[m - 1]));
    ...
    if (log_ratios_old.size() == log_ratios_new.size() &&
      ...
      for (size_t i = 0; i < min_size; ++i) {
        ...
        max_rel =
            isfinite_val(diff) ? max_val(max_rel, diff) : pos_inf_val<T>();
    ...
    if (debug) {
      ...
      auto iter_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                         iter_end - iter_start)
                         .count();
      std::cerr << "[DEBUG] Graeffe iter " << iter
                << ": max_rel = " << static_cast<double>(max_rel)
                << ", time = " << iter_ms << " ms" << std::endl;
    }

    // Проверка критериев остановки: используем isfinite вместо сравнения с
    // 1e100
    ...
    if (max_rel < epsilon) {
      if (debug) {
        std::cerr << "[DEBUG] Graeffe: сходимость достигнута на итерации "
                  << iter << std::endl;
      }
      converged = true;
      break;
  ...
  // Если некоторые средние коэффициенты равны нулю (или -∞ в лог-форме),
  // это означает, что все корни имеют одинаковый модуль.
  ...
  if (has_zero_middle) {
    ...
    if (is_finite_ld(L_first) && is_finite_ld(L_last)) {
      ...
      // Тернарник: если модуль неконечный или отрицательный, заменяем на 0
      common_modulus = (!is_finite_ld(common_modulus) || common_modulus < T(0))
                           ? T(0)
                           : common_modulus;
  ...
  if (debug) {
    ...
    std::cerr << "[DEBUG]   n = " << n << ", iters_done = " << iters_done
              << ", pow_two_k = " << static_cast<double>(pow_two_k)
              << std::endl;
  ...
  for (size_t j = 1; j <= n; ++j) {
    ...
    if (!is_finite_ld(Lprev) && !is_finite_ld(Lcurr)) {
      if (debug) {
        std::cerr << "[DEBUG]   j=" << j << ": оба -inf, модуль = 0"
                  << std::endl;
      }
    ...
    // ВАЖНО: если Lcurr = -inf (нулевой коэффициент при младшей степени),
    // это означает что полином делится на x, т.е. один из корней = 0.
    // В этом случае модуль этого корня = 0, а не подставлять LOG_TINY.
    if (!is_finite_ld(Lcurr)) {
      if (debug) {
        std::cerr << "[DEBUG]   j=" << j
                  << ": Lcurr=-inf (нулевой коэфф.), модуль = 0" << std::endl;
      }
      moduli.push_back(T(0));
      continue;
    }

    // Подстановка для -inf только для Lprev: если предыдущий коэффициент
    // был нулевой, но текущий ненулевой, используем LOG_TINY для стабильности
    ...
    if (debug) {
      std::cerr << "[DEBUG]   j=" << j
                << ": Lprev=" << static_cast<double>(Ldesc[j - 1])
                << ", Lcurr=" << static_cast<double>(Ldesc[j])
                << ", log_modulus=" << static_cast<double>(log_modulus)
                << std::endl;
    }
    ...
    // Тернарник вместо if для устранения ветвления
    modulus = (isfinite_val(modulus) && modulus >= T(0)) ? modulus : T(0);

    if (debug) {
      std::cerr << "[DEBUG]   j=" << j
                << ": modulus=" << static_cast<double>(modulus) << std::endl;
    }
...
template <RealNumber T> struct ModulusWithMultiplicity {
  ...
  ModulusWithMultiplicity(T val, int mult = 1)
      : value(val), multiplicity(mult) {}
};
...
find_moduli_with_multiplicities_by_graeffe(
    ...
    int maxIter = numeric_constants::DEFAULT_MAX_ITERATIONS,
    bool debug = false) {
  std::cerr << "[TRACE] Entering find_moduli_with_multiplicities_by_graeffe"
            << std::endl;
  // Сначала находим модули стандартным методом
  std::vector<T> raw_moduli =
